...
RUN echo 'deb http://mirrors.ustc.edu.cn/CRAN/bin/linux/ubuntu xenial/' >> /etc/apt/sources.list; \
    ...
    apt-get update; \
    apt-get install -y r-base r-base-dev libcurl4-openssl-dev libxml2-dev gdebi-core libapparmor1 psmisc supervisor libedit2 wget libssl-dev; \
    apt-get clean; \
    apt-get autoremove
...
RUN R -e "install.packages(c('shiny', 'rmarkdown', 'tidyverse', 'gplots', 'plotrix', 'VennDiagram', 'httr'), repos='http://mirrors.ustc.edu.cn/CRAN/')"
